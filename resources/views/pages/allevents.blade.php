       ...
       <div class="product_area">
            <div class="container">
                ...
                <div class="porduct_wrapper">
                    <?php
                        $getProductMode=App\Product::where('status',1)->where('occasional',$event['id'])->select('product_mode')->groupBy('product_mode')->orderBy('id','DESC')->get()->toArray();
                    ?>
                    ...
                    @foreach($getProductMode as $mode)
                    <?php
                        $getpros=array();
                        if(!empty($mode['product_mode'])){
                            $getpros=App\Product::where(['product_mode'=>$mode['product_mode']])->inRandomOrder()->orderBy('id','DESC')->get()->toArray();
                        }
