        ...
        <div class="container">
            <div class="porduct_wrapper">
                @foreach($getSection as $section)
                    @if($getBarnd>0)
                        @foreach($getBarnd as $brand)
                        <?php
                            $getprosCount=App\Product::where('brand_id',$brand['id'])->where('section_id',$section['id'])->inRandomOrder()->orderBy('id','DESC')->count();
                        ?>
                        @if($getprosCount>0)
                        <div class="product_wrpper_hearder row">
                            <div class="col-sm-6 col-10">
                                <h2 class="prw_title">{{Str::upper($brand['brand_name'])}}</h2>
                            </div>
                        </div>
                        <?php
                            $getpros=App\Product::where('brand_id',$brand['id'])->where('section_id',$section['id'])->inRandomOrder()->orderBy('id','DESC')->get()->toArray();
                        ?>
                        <div class="product_inner brand-carousel owl-carousel owl-theme owl-loaded">
                       @foreach($getpros as $row) 
                        <?php
                        ...
                        <?php
                            ...
                            $productSizes =App\AttributeProduct::where('product_id',$row['id'])->where('status',1)->get()->toArray();
