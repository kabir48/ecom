...
<div class="container" id="example_id">
    <div class="row">
        <div class="col-xs-12">
    		<div class="invoice-title">
                <h2 class="text-center" style="font-weight: bold"><img src="{{asset('public/media/logo/'.$sitesetting->logo)}}" alt="" wisth="60"></h2>
                <h3 class="text-center">  {{$sitesetting->company_name}}</h3>
...
<button id="dl-png" class="btn first">Download This Invoice</button>
<script src="{{asset('public/frontend/assets/js/html2canvas.min.js')}}"></script>
<script>
    document.getElementById("dl-png").onclick=function(){
        const sceenshotTarget=document.getElementById("example_id");

        html2canvas(sceenshotTarget).then((canvas)=>{
            const base64image = canvas.toDataURL("image/png");
            var anchor = document.createElement('a');
            anchor.setAttribute("href",base64image);
            anchor.setAttribute("download",{{$orderInvoice->order_no}}
            );
            anchor.click();
            anchor.remove();
        });
