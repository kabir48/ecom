        ...
        <div class="container">
            <div class="porduct_wrapper">
                ...
                <div class="product_inner men-category-slider owl-carousel owl-theme owl-loaded">
                      @foreach($catProductMode as $data)
                        <?php
                        ...
                        <?php
                            ...
                            if(!empty($data['group_code'])){
                                $groupProducts=App\Product::where(['group_code'=>$data['group_code'],'status'=>1])->get()->toArray();
                                //dd($groupProducts);die;
                            }
                            $productSizes =App\AttributeProduct::where('product_id',$data['id'])->where('status',1)->get();
                             $productColor =App\AttributeProduct::where('product_id',$data['id'])->where('status',1)->first();
                        ?>
                    <div class="product_box item quick_product_data @if($qty == 0 || $total_stock==0) product-disable @else   @endif">
                        <div class="productVariations">
                             <div class="productVariation" id="variation-id-1">
                                        <!-- product Image box -->
                                <div class="image_box">
                                    @if($discounted_price>0)
                                    <h4 class="product_tag">{{Str::ucfirst('on sale')}}</h4>
                                ...
                                <!-- Product Content Box -->
                                <div class="product_meta_box">
                                    <h4 class="product_color product_color_name" id="color-{{$data['id']}}" color_id="{{$data['id']}}">{{$productColor->color}}</h4>
                                    <a href="{{url('product/details/'.$data['id'].'/'.$data['product_name'])}}">
