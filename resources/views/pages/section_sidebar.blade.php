    ...
    <?php
        $getSectionSizes=ProductFilter::getSectionSizes($url);
        $getSectionColors=ProductFilter::getSectionColors($url);
        $getSectionBrands=ProductFilter::getSectionBrands($url);
    ?> 
    ...
    <div class="col-12 col-sm-12 col-md-3 col-lg-3 sidebar filterbar">
		...
		<div class="sidebar_tags">
			...
			<div class="sidebar_widget categories filter-widget">
				...
				<div class="widget-content" style="">
					<ul class="sidebar_categories">
					    <?php
                           $sections=Section::with('categories')->where('url',$url)->get()->toArray();
                        ?>
                        @if(count($sections)>0)
                        @foreach ($sections as $section)
                        @if (count($section['categories'])>0)
                        @foreach($section['categories'] as $category)
						<li class="level1 sub-level"><a href="{{url('category-products/'.$category['url'])}}" class="site-nav toggle-sublinks">{{$category['category_name']}} ({{App\Product::where('category_id',$category['id'])->count()}})</a>
							<ul class="sublinks" style="display: none;">
								@if(count($category['subcategories'])>0)
                                @foreach($category['subcategories'] as $catB)
			...
			<div class="sidebar_widget filterBox filter-widget brand-filter">
				...
				<ul>
				    @foreach($getSectionSizes as $key=>$size)
					<li>
			...
			<div class="sidebar_widget filterBox filter-widget brand-filter">
				...
				<ul>
				    @forelse($getSectionColors as $key=>$data)
                    <?php
			...
			<?php
               $filterAvailable=ProductFilter::filterAvailable($filter['id'],$sectionDetails['sectionDetails']['id']);
            ?>
			...
			<!--====Product Filter End====-->
			<?php
			   $uniqueProductIds = App\OrderDetail::distinct()->pluck('product_id');
			   $getProducts=App\Product::with(['category','section'])->whereIn('id',$uniqueProductIds)->where('section_id',$sectionDetails['sectionDetails']['id'])->latest()->take(8)->get()->toArray();
			?>
			<!--====Popular Products====-->
